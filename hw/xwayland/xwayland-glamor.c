...
#endif
//#ifdef XWL_HAS_GLAMOR_HYBRIS
    //xwl_glamor_init_glamor_hybris(xwl_screen);
    xwl_glamor_init_hybris(xwl_screen);
    //if (!xwl_screen->glamor_hybris_backend.is_available && use_hybris)
    //ErrorF("xwayland glamor: glamor hybris backend requested and available\n");
//#endif
}
...
//#ifdef XWL_HAS_GLAMOR_HYBRIS
    if (xwl_screen->glamor_hybris_backend.is_available ) {
        ErrorF("glamor: Using glamor hybris interface, direct rendering impossible.\n");
        ErrorF("glamor: Performance may be affected. Ask your vendor to support GBM!\n");
